...
export class JobDispatcher {
  ...
  async sendEmail(payload: SendEmailJobPayload, options?: JobsOptions) {
    ...
    return this.emailQueue.add(JOB_NAMES.SEND_GENERIC_EMAIL, payload, options);
  }

  async sendWelcomeEmail(payload: Pick<SendEmailJobPayload, 'to' | 'context'>) {
  ...
  async sendPasswordReset(payload: Pick<SendEmailJobPayload, 'to' | 'context'>) {
    return this.emailQueue.add(
      JOB_NAMES.SEND_PASSWORD_RESET,
      { ...payload, subject: 'Password Reset', template: 'password-reset' },
  ...
  async generateReport(
    ...
    return this.reportQueue.add(JOB_NAMES.GENERATE_REPORT, { type, params }, options);
  }

  async processFileUpload(fileId: string, options?: JobsOptions) {
    return this.fileQueue.add(JOB_NAMES.PROCESS_UPLOAD, { fileId }, options);
  }
  ...
  async dispatchWebhook(
    ...
    return this.webhookQueue.add(
      ...
      { url, event, payload },
      { attempts: 5, backoff: { type: 'exponential', delay: 10000 }, ...options },
    );
  }
